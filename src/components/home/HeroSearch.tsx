...
export function SearchSection({ className }: { className?: string }) {
  ...
  const { meta } = useSelector((state: any) => state?.search);
  ...
  // ─── Re-run search whenever page or limit changes (pagination navigation) ───
  useEffect(() => {
    if (!lastSearchParams.current) return; // no search has been submitted yet
  ...
  // ─── Core search executor (shared by form submit + pagination re-trigger) ───
  const runSearch = async (
    ...
    try {
      ...
      setTimeout(() => dispatch(setSearchLoading(false)), 1000);
    }
  };

  // ─── Form submit: snapshot params, reset to page 1, then search ───
...
//   const limit = useSelector((state: any) => state?.search?.limit);
//   const { meta } = useSelector((state: any) => state?.search);
