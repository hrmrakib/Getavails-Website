...
export default function UpdateSubscriptionPage() {
  ...
  useEffect(() => {
    if (!isLoading && subscriptionsResponse?.data) {
      setFormData({
        name: subscriptionsResponse?.data?.name,
        price: subscriptionsResponse?.data?.price,
        subscription_interval:
          subscriptionsResponse?.data?.subscription_interval,
        isHot: subscriptionsResponse?.data?.isHot,
      });
      setSelectedFeatures(subscriptionsResponse?.data?.features);
    }
  }, [isLoading, subscriptionsResponse]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateSubscriptionMutation(payload).unwrap();
      if (res?.success) {
